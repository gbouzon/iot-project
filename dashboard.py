...
app.layout = html.Div(id="theme-switch-div", children=[
    navbar, 
    content,
    dcc.Interval(id='temperature-interval', interval=3*1000, n_intervals=0),
    dcc.Interval(id='humidity-interval', interval=3*1000, n_intervals=0)
]);
...
@app.callback(Output('temperature-thermometer', 'value'),
...
def update_sensor(n, tValue):
     ...
     if temperatureValue > float(uTempThreshold) and not emailSent:
         ...
         time.sleep(5)
         toggle_fan(False)
     elif temperatureValue < float(uTempThreshold):
         toggle_fan(False)
         emailSent = False
         emailReceived = 0
     ...
     return temperatureValue, tempUnit
